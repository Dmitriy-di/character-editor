...
//   private mCharacter: AbstractMesh; // отредактированный
...
//   private mIsUpdatableVertecies: boolean; // являются ли массивы вершин обновляемыми
//   // конструктор
//   constructor(inScene: Scene) {
//     this.mScene = inScene;
...
//     this.mAthletic.position = new Vector3(-1, 0, 0);
...
//     this.mNormal.position = new Vector3(-2, 0, 0);
...
//   private async loadModel(inFilename: string) {
//     const res = await SceneLoader.ImportMeshAsync("", "assets/models/Characters/", inFilename, this.mScene);
...
//   changeModel(inSkinnyVal: number, inFatVal: number, inAthleticVal: number) {
//     console.log("inSkinnyVal = " + inSkinnyVal + "; inFatVal = " + inFatVal + "; inAthleticVal = " + inAthleticVal);
...
//       this.changeMesh(meshes[i], meshesN[i], meshesS[i], meshesF[i], meshesA[i], sVal, fVal, aVal);
//     }
...
//   changeMesh(inMesh: AbstractMesh, inMeshN: AbstractMesh, inMeshS: AbstractMesh, inMeshF: AbstractMesh, inMeshA: AbstractMesh, inSkinnyVal: number, inFatVal: number, inAthleticVal: number) {
...
//           positions[i2] = positionsN[i2] + (positionsS[i2] - positionsN[i2]) * inSkinnyVal;
//           positions[i3] = positionsN[i3] + (positionsS[i3] - positionsN[i3]) * inSkinnyVal;
//           // сдвинутый к худому сдвигаем к толстому
//           positions[i] = positions[i] + (positionsF[i] - positions[i]) * inFatVal;
//           positions[i2] = positions[i2] + (positionsF[i2] - positions[i2]) * inFatVal;
//           positions[i3] = positions[i3] + (positionsF[i3] - positions[i3]) * inFatVal;
//           // сдвинутый к толстому сдвигаем к атлетичному
//           positions[i] = positions[i] + (positionsA[i] - positions[i]) * inAthleticVal;
//           positions[i2] = positions[i2] + (positionsA[i2] - positions[i2]) * inAthleticVal;
//           positions[i3] = positions[i3] + (positionsA[i3] - positions[i3]) * inAthleticVal;
//         }
//         // нужно новый буффер создать, так как прошлый был не обновляемым
//         if(this.mIsUpdatableVertecies)
//           inMesh.updateVerticesData(VertexBuffer.PositionKind, positions, true, true)
//         else
//           inMesh.setVerticesData(VertexBuffer.PositionKind, positions, true);
...
//       this.changeMesh(meshes[j], meshesN[j], meshesS[j], meshesF[j], meshesA[j], inSkinnyVal, inFatVal, inAthleticVal);
//     }
//   }
// }